...
export default function EditCategoryPage() {
  ...
  const loadCategory = async () => {
    try {
      const category = await categoriesApi.getOne(params.id as string)
      setFormData({
        name: category.name,
        description: category.description,
        slug: category.slug,
        image: category.image || { url: "", base64: "" },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await categoriesApi.update(params.id as string, formData)
      toast({
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">{t.categories.editCategory}</h1>
        </div>
      </div>
